...
subplot1.legend(loc="lower center", bbox_to_anchor=(0.5, -0.25), ncol=2, fancybox=true, shadow=true)
...
subplot2.legend(loc="lower center", bbox_to_anchor=(0.5, -0.25), ncol=2, fancybox=true, shadow=true)

subplot3.plot(time, avg_rho, linestyle="-", label="With Endpoint")
subplot3.plot(time, tl_avg_rho, label="trend (gradient=$(@sprintf("%.5f", grad_avg_rho)))",
     color="red", linewidth=2)
subplot3.set_xlabel("Time [s]")
subplot3.set_ylabel("Avg. Density [kg/m^3]")
subplot3.set_title("Avg. Density of the Fluid")
subplot3.legend(loc="lower center", bbox_to_anchor=(0.5, -0.25), ncol=2, fancybox=true, shadow=true)
