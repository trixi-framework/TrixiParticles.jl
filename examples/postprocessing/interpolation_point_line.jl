...
println(
    interpolate_points(
        [position_x; 0.01;;], semi, fluid_system, sol,
...
println(
    interpolate_points(
        [
            position_x position_x position_x position_x position_x;
...
result_endpoint = interpolate_line(
    start_point, end_point, n_interpolation_points,
...
p = Plots.plot(
    walldistance, result.density, marker = :circle, color = :blue,
    markerstrokecolor = :blue, linewidth = 2, label = "With Endpoint"
)

Plots.plot!(
    p, walldistance_endpoint, result_endpoint.density, marker = :xcross, linewidth = 2,
    linestyle = :dash, label = "Without Endpoint", color = :orange
)

Plots.plot!(
    p, framestyle = :box, legend = :best, xlabel = "Wall distance", ylabel = "Density",
