...
original_plane = interpolate_plane_2d(
    interpolation_start, interpolation_end, resolution,
...
double_smoothing_plane = interpolate_plane_2d(
...
half_smoothing_plane = interpolate_plane_2d(
    interpolation_start, interpolation_end,
    resolution, semi, fluid_system, sol,
...
scatter1 = Plots.scatter(
    original_x, original_y, zcolor = original_pressure, marker = :circle,
    markersize = 2, markercolor = :viridis, markerstrokewidth = 0
)
scatter2 = Plots.scatter(
    double_x, double_y, zcolor = double_pressure, marker = :circle,
    markersize = 2, markercolor = :viridis, markerstrokewidth = 0
)
scatter3 = Plots.scatter(
    half_x, half_y, zcolor = half_pressure, marker = :circle, markersize = 2,
    markercolor = :viridis, markerstrokewidth = 0
)

plot1 = Plots.plot(
    scatter1, xlabel = "X Coordinate", ylabel = "Y Coordinate",
    title = "Pressure Distribution", colorbar_title = "Pressure",
    ylim = (0.0, 1.0), legend = false, clim = (0, 9000), colorbar = true
)
plot2 = Plots.plot(
    scatter2, xlabel = "X Coordinate", ylabel = "Y Coordinate",
    title = "Pressure with 2x Smoothing Length", colorbar_title = "Pressure",
    ylim = (0.0, 1.0), legend = false, clim = (0, 9000), colorbar = true
)
plot3 = Plots.plot(
    scatter3, xlabel = "X Coordinate", ylabel = "Y Coordinate",
...
original_plane = interpolate_plane_3d(
    p1, p2, p3, resolution, semi,
    fluid_system, sol
)
original_x = original_plane.point_coords[1, :]
original_y = original_plane.point_coords[2, :]
...
scatter_3d = Plots.scatter3d(
    original_x, original_y, original_z, marker_z = original_pressure,
    color = :viridis, markerstrokewidth = 0
)

plot_3d = Plots.plot(
    scatter_3d, xlabel = "X", ylabel = "Y", zlabel = "Z",
    title = "3D Scatter Plot with Pressure Coloring", legend = false,
    clim = (0, 9000), colorbar = false
)

combined_plot = Plots.plot(
    plot1, plot2, plot3, plot_3d, layout = (2, 2),
...
save_interpolation_cb = PostprocessCallback(;
    dt = 0.1, write_file_interval = 0,
