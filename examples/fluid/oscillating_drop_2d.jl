...
fluid = SphereShape(
    fluid_particle_spacing, radius, (0.0, 0.0),
...
fluid_system = WeaklyCompressibleSPHSystem(
    fluid, fluid_density_calculator,
...
sol = solve(
    ode, RDPK3SpFSAL49(),
    abstol = 1.0e-7, # Default abstol is 1e-6 (may need to be tuned to prevent intabilities)
    reltol = 1.0e-4, # Default reltol is 1e-3 (may need to be tuned to prevent intabilities)
