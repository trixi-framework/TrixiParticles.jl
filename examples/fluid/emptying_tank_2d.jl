...
# ==========================================================================================
# ==== Experiment Setup
# TODO: filling Tank is not working since the velocity is set.
tspan = (0.0, 4.0)
...
pipe = RectangularTank(particle_spacing, domain_size.+((boundary_layers+1)*particle_spacing,0.0), boundary_size, fluid_density,
                       pressure=pressure, n_layers=boundary_layers,
                       faces=(false, false, true, true))

tank = RectangularTank(particle_spacing, (1.5-boundary_layers*particle_spacing, 1.0), (1.5, 1.0), fluid_density,
                       pressure=pressure, n_layers=boundary_layers,
                       faces=(false, true, true, true),
                       min_coordinates=(1.0,-0.6))

tank_wall_left = RectangularShape(particle_spacing,
                                  (boundary_layers,
                                   round(Int, (1.0 - domain_size[2]) / particle_spacing)),
                                  (1.0, -0.6), density=fluid_density)


tank.fluid.coordinates[1, :] .+= particle_spacing * boundary_layers
fluid = union(pipe.fluid, tank.fluid)

# Shift pipe walls in negative x-direction for the inflow
# pipe.boundary.coordinates[1, :] .-= particle_spacing * open_boundary_layers

n_buffer_particles = 200 * pipe.n_particles_per_dimension[2]
...
#     if t > 4
#         return SVector(max((4-t) * prescribed_velocity, 0.0), 0.0)
#     else
...
#                                          reference_velocity=[prescribed_velocity, 0.0])

outflow = OutFlow(; plane=([0.0, 0.0], [0.0, domain_size[2]]),
...
tank_boundary_model = BoundaryModelDummyParticles(tank.boundary.density, tank.boundary.mass,
                                                  AdamiPressureExtrapolation(),
                                                  state_equation=state_equation,
                                                  #viscosity=ViscosityAdami(nu=1e-4),
                                                  smoothing_kernel, smoothing_length)

tank_boundary_system = BoundarySPHSystem(tank.boundary, tank_boundary_model)

wall_boundary_model = BoundaryModelDummyParticles(tank_wall_left.density,
                                                  tank_wall_left.mass,
                                                  AdamiPressureExtrapolation(),
                                                  state_equation=state_equation,
                                                  #viscosity=ViscosityAdami(nu=1e-4),
...
wall_boundary_system = BoundarySPHSystem(tank_wall_left, wall_boundary_model)

# ==========================================================================================
# ==== Simulation
semi = Semidiscretization(fluid_system, open_boundary_out, boundary_system,
                          tank_boundary_system, wall_boundary_system)
