...
packing_system = ParticlePackingSystem(
    shape_sampled; smoothing_length = smoothing_length,
    signed_distance_field, tlsph = tlsph,
    background_pressure
)

boundary_system = ParticlePackingSystem(
    boundary_sampled; smoothing_length = smoothing_length,
...
steady_state = SteadyStateReachedCallback(;
    interval = 10, interval_size = 200,
...
pp_cb_ekin = PostprocessCallback(;
    ekin = kinetic_energy, interval = 1,
    filename = "kinetic_energy", write_file_interval = 50
)

callbacks = CallbackSet(
    UpdateCallback(), saving_callback, info_callback, steady_state,
    pp_cb_ekin
)

sol = solve(
    ode, RDPK3SpFSAL35();
    save_everystep = false, maxiters = 1000, callback = callbacks, dtmax = 1.0e-2
)
