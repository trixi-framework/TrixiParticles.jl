...
function pixie_ode_solve(ode, method, tspan; abstol=1e-6, reltol=1e-3, dtmax=1e-2, save_everystep=false, callbacks=nothing, save_when_error=true, save=false, alive_interval=100, save_interval=0.02)
    ...
    sol = nothing
    try
        sol = solve(ode, method, abstol=abstol, reltol=reltol, dtmax=dtmax, save_everystep=save_everystep, callback=callbacks);
    catch e
        if save_when_error
