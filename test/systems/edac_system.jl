@testset verbose = true "EntropicallyDampedSPHSystem" begin
    ...
    @trixi_testset "Constructors" begin
        ...
        @testset "$(i + 1)D" for i in 1:2
            ...
            @test_throws ArgumentError(error_str1) EntropicallyDampedSPHSystem(
                initial_condition,
                smoothing_kernel,
                smoothing_length,
                sound_speed,
            ...
            @test_throws ArgumentError(error_str2) EntropicallyDampedSPHSystem(
                initial_condition,
                smoothing_kernel2,
                smoothing_length,
                sound_speed
    ...
    @trixi_testset "Constructors with Setups" begin
        setups = [
            ...
            RectangularTank(
                0.123, (0.369, 0.246, 0.246), (0.369, 0.492, 0.492),
                1020.0
            ).fluid,
        ...
        @testset "$(setup_names[i])" for i in eachindex(setups)
            ...
            system = EntropicallyDampedSPHSystem(
                setup, smoothing_kernel, smoothing_length,
                sound_speed
            )
        ...
        @testset "Wrong acceleration dimension" for i in eachindex(NDIMS_)
            ...
            @test_throws ArgumentError(error_str) EntropicallyDampedSPHSystem(
                setup,
                smoothing_kernel,
                smoothing_length,
                sound_speed,
    ...
    @trixi_testset "Average Pressure" begin
        ...
        TrixiParticles.update_average_pressure!(
            system, system.transport_velocity, v_ode,
            u_ode, semi
        )

        @test all(
            i -> system.cache.neighbor_counter[i] == nparticles(system),
            nparticles(system)
        )
        @test all(
            i -> isapprox(system.cache.pressure_average[i], -50.968532955185964),
            nparticles(system)
        )
    end
end
