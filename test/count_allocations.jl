...
function copy_semi_with_no_update_nhs(semi)
    neighborhood_searches = Tuple(
        Tuple(
                NoUpdateNeighborhoodSearch(nhs)
                for nhs in searches
            )
...
# No update
@inline function PointNeighbors.update!(
        search::NoUpdateNeighborhoodSearch, x, y;
...
function count_rhs_allocations(sol, semi)
    ...
    try
        ...
        return @invokelatest count_rhs_allocations_inner(
            dv_ode, du_ode, v_ode, u_ode,
...
@inline function count_rhs_allocations_inner(
        dv_ode, du_ode, v_ode, u_ode,
        semi_no_nhs_update, t
    )
    ...
    # Count allocations
    allocations_kick = @allocated TrixiParticles.kick!(
        dv_ode, v_ode, u_ode,
        semi_no_nhs_update, t
    )
    allocations_drift = @allocated TrixiParticles.drift!(
        du_ode, v_ode, u_ode,
        semi_no_nhs_update, t
    )
