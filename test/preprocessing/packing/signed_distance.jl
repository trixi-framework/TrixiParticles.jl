@testset verbose = true "Signed Distance Field" begin
    @testset verbose = true "`show`" begin
        ...
        signed_distance_field = SignedDistanceField(
            geometry, 0.1; max_signed_distance = 0.45,
    ...
    @testset verbose = true "Real World Data" begin
        ...
        @testset verbose = true "$(files[i])" for i in eachindex(files)
            ...
            @testset verbose = true "Positions $j" for j in eachindex(positions_test)
                ...
                normals = fill(
                    SVector(ntuple(dim -> Inf, ndims(geometry))),
                    length(positions)
                )
            ...
            @testset verbose = true "Pointcloud" begin
                ...
                @test isapprox(signed_distance_field.positions, points; rtol = 1.0e-4)
                @test isapprox(signed_distance_field.normals, vertex_normals; rtol = 1.0e-4)
                @test isapprox(signed_distance_field.distances, distances; rtol = 1.0e-4)
            end
        end
    end
end
