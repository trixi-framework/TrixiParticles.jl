@testset verbose = true "FaceNeighborhoodSearch" begin
    @testset verbose = true "2D Cell Bounding Box" begin
        ...
        @testset verbose = true "Axis Aligned Edge: cell size $(cell_size)" for (
                i,
                cell_size,
            ) in
    ...
    @testset verbose = true "3D Cell Bounding Box" begin
        ...
        triangle_aligned = TrixiParticles.TriangleMesh(
            [(A, B, C)],
        ...
        @testset verbose = true "Axis Aligned Triangle: cell size $(cell_size)" for (
                i,
                cell_size,
            ) in
            enumerate(cell_sizes)
        ...
        triangle_arbitrary = TrixiParticles.TriangleMesh(
            [(A, B, C)],
            [
                TrixiParticles.cross(
                    B - A,
                    C - A
                ),
            ],
        ...
        @testset verbose = true "Arbitrary Triangle: cell size $(cell_size)" for (
                i,
                cell_size,
            ) in
            enumerate(cell_sizes)
            ...
            @test expected_ncells_bbox[i] ==
                size(
                collect(
                    TrixiParticles.bounding_box(
                        face_id, triangle_arbitrary,
                        nhs
                    )
                )
            )
