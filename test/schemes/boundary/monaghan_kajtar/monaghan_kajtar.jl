@testset verbose = true "Monghan-Kajtar Repulsive Particles" begin
    ...
    @testset "RHS" begin
        ...
        fluid = rectangular_patch(
            particle_spacing, (3, 3), perturbation_factor = 0.0,
        ...
        boundary = rectangular_patch(
            2particle_spacing, (1, 3), perturbation_factor = 0.0,
            perturbation_factor_position = 0.0
        )
        K = 1.0
        spacing_ratio = 0.5
        boundary_model = BoundaryModelMonaghanKajtar(
            K, spacing_ratio, 2particle_spacing,
            boundary.mass
        )
        ...
        TrixiParticles.interact!(
            dv, v, u, v_neighbor, u_neighbor, fluid_system,
            boundary_system, semi
        )

        # Due to the symmetric setup, all particles will only be accelerated horizontally
        @test isapprox(dv[2, :], zeros(TrixiParticles.nparticles(fluid_system)), atol = 1.0e-14)
