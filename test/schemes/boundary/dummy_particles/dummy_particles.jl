@testset verbose = true "Dummy Particles" begin
    ...
    @testset verbose = true "Viscosity Adami/Bernoulli: Wall Velocity" begin
        ...
        boundary_model_adami = BoundaryModelDummyParticles(
            boundary.density,
            boundary.mass,
        ...
        boundary_model_bernoulli = BoundaryModelDummyParticles(
            boundary.density,
            boundary.mass,
            state_equation = state_equation,
        ...
        boundary_systems = [
            ...
            TotalLagrangianSPHSystem(
                boundary, smoothing_kernel,
                smoothing_length, 1.0e6, 0.3;
        ...
        for boundary_system in boundary_systems
            ...
            @testset "$system_name with $density_calculator" begin
                ...
                @testset "Wall Velocity $v_fluid" for v_fluid in velocities
                    ...
                    @test isapprox(
                        boundary_model.cache.wall_velocity,
                        v_wall
                    )
                end
    ...
    @testset "Pressure Extrapolation Adami" begin
        ...
        state_equation = StateEquationCole(
            sound_speed = 10, reference_density = 257,
            exponent = 7
        )

        tank1 = RectangularTank(
            particle_spacing, (width, height), (width, height),
            density, n_layers = n_layers,
            faces = (true, true, true, false)
        )

        boundary_model = BoundaryModelDummyParticles(
            tank1.boundary.density,
            tank1.boundary.mass,
            state_equation = state_equation,
            AdamiPressureExtrapolation(),
        ...
        @testset "Constant Zero Pressure" begin
            fluid_system1 = WeaklyCompressibleSPHSystem(
                tank1.fluid, SummationDensity(),
                state_equation,
                smoothing_kernel, smoothing_length
            )
            ...
            for particle in TrixiParticles.eachparticle(boundary_system)
                TrixiParticles.compute_adami_density!(
        ...
        @testset "Constant Non-Zero Pressure" begin
            density = 260
            tank2 = RectangularTank(
                particle_spacing, (width, height), (width, height),
                density, n_layers = n_layers,
                faces = (true, true, true, false)
            )

            fluid_system2 = WeaklyCompressibleSPHSystem(
                tank2.fluid, SummationDensity(),
                state_equation,
                smoothing_kernel, smoothing_length
            )
            ...
            @test all(
                isapprox.(
                    boundary_system.boundary_model.pressure,
        ...
        @testset "Hydrostatic Pressure Gradient" begin
            for flipped_condition in (Val(true), Val(false))
                ...
                fluid_system3 = WeaklyCompressibleSPHSystem(
                    tank3.fluid, SummationDensity(),
                    state_equation,
                    smoothing_kernel,
                    smoothing_length,
                ...
                tank_reference = RectangularTank(
                    particle_spacing,
                ...
                function set_pressure!(
                        pressure, coordinates, offset, system,
                        system_pressure
                    )
                ...
                set_pressure!(
                    pressure, boundary_system.coordinates, n_layers + 0.5,
                ...
                # so the boundary starts at -(n_layers + 0.5) * particle_spacing
                set_pressure!(
                    pressure, tank3.fluid.coordinates, n_layers + 0.5,
                ...
                # The fluid starts at -0.5 * particle_spacing from (0, 0)
                set_pressure!(
                    pressure_reference, tank_reference.fluid.coordinates, 0.5,
