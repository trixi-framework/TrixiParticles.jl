@testset verbose = true "Dummy Particles RHS" begin
    ...
    @testset verbose = true "Momentum and Total Energy Conservation" begin
        ...
        function second_systems(
                initial_condition, density_calculator, state_equation,
                smoothing_kernel, smoothing_length
            )
            second_fluid_system = WeaklyCompressibleSPHSystem(
                initial_condition,
                density_calculator,
                state_equation,
                smoothing_kernel,
            ...
            boundary_model_zeroing = BoundaryModelDummyParticles(
            ...
            boundary_system_zeroing = BoundarySPHSystem(
                initial_condition,
                boundary_model_zeroing
            )
            boundary_model_continuity = BoundaryModelDummyParticles(
                initial_condition.density,
                initial_condition.mass,
                ContinuityDensity(),
                smoothing_kernel,
            ...
            boundary_system_continuity = BoundarySPHSystem(
                initial_condition,
                boundary_model_continuity
            )

            boundary_model_summation = BoundaryModelDummyParticles(
                initial_condition.density,
                initial_condition.mass,
                SummationDensity(),
                smoothing_kernel,
            ...
            boundary_system_summation = BoundarySPHSystem(
                initial_condition,
            ...
            vu = Dict(
                ...
                "Fluid-BoundaryDummyContinuityDensity" => (
                    v_boundary_continuity,
                    u_boundary,
                ),
        ...
        @testset "`$(nameof(typeof(density_calculator)))`" for density_calculator in
            ...
            for seed in 1:3
                ...
                fluid = InitialCondition{ndims(ic)}(
                    ic.coordinates[:, 1:center_particle],
                ...
                boundary = InitialCondition{ndims(ic)}(
                    ic.coordinates[
                        :,
                ...
                @testset "$key" for key in keys(systems)
                    ...
                    TrixiParticles.interact!(
                        dv_neighbor, v_neighbor, u_neighbor, v, u,
                    ...
                    if neighbor_system isa WeaklyCompressibleSPHSystem
                        ...
                        TrixiParticles.interact!(
                            dv_neighbor, v_neighbor, u_neighbor,
                    ...
                    function drho(
                            dv, ::ContinuityDensity, system, neighbor_system,
                    ...
                    function drho(
                            dv, ::SummationDensity, system, neighbor_system,
                            v, u, v_neighbor, u_neighbor, particle
                        )
                        sum1 = sum(
                            neighbor -> drho_particle(
                                system, system,
                                v, u, v, u,
                                particle, neighbor
                            ),
                    ...
                    function drho(
                            dv, ::PressureZeroing, system, neighbor_system,
                            v, u, v_neighbor, u_neighbor, particle
                        )
                        return 0.0
                    end
                    ...
                    function drho(
                            dv, ::PressureMirroring, system, neighbor_system,
                            v, u, v_neighbor, u_neighbor, particle
                        )
                        return 0.0
                    end
                    ...
                    function drho_particle(
                        ...
                        v_diff = TrixiParticles.current_velocity(v, system, particle) -
                            TrixiParticles.current_velocity(
                            v_neighbor,
                            neighbor_system, neighbor
                        )

                        pos_diff = TrixiParticles.current_coords(u, system, particle) -
                            TrixiParticles.current_coords(
                            u_neighbor,
                            neighbor_system, neighbor
                        )
                        ...
                        grad_kernel = TrixiParticles.smoothing_kernel_grad(
                            fluid_system,
                            pos_diff,
                            distance,
                            particle
                    ...
                    # ∑ m_a (v_a ⋅ dv_a + dte_a)
                    deriv_total_energy = sum(
                        particle -> deriv_energy(
                            dv, v, u,
                            v_neighbor,
                            u_neighbor,
                            fluid_system,
                    ...
                    @test isapprox(deriv_total_energy, 0.0, atol = 6.0e-15)
                end
            end
        end
    end
