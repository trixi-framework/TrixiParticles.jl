                ...
                @testset "`$(nameof(typeof(pressure_acceleration)))`" for pressure_acceleration in
                    ...
                    for (m_a, m_b) in masses, (rho_a, rho_b) in densities,
                        ...
                        @testset verbose = true "$system_name" for system_name in [
                            ...
                            dv1 = TrixiParticles.pressure_acceleration(
                                system, system,
                                -1, -1,
                                m_a, m_b, p_a, p_b,
                                rho_a, rho_b,
                            ...
                            dv2 = TrixiParticles.pressure_acceleration(
                                system, system,
                                -1, -1,
                                m_b, m_a, p_b, p_a,
                                rho_b, rho_a,
    ...
    @testset verbose = true "Momentum and Total Energy Conservation" begin
        ...
        @testset "`$(nameof(typeof(density_calculator)))`" for density_calculator in
            ...
            for seed in 1:3
                ...
                @testset "`$(nameof(typeof(system)))`" for system in (
                        system_wcsph,
                        system_edac,
                    )
                    u = fluid.coordinates
                    ...
                    @test isapprox(deriv_linear_momentum, zeros(2, 1), atol = 5.0e-14)
                    ...
                    @test isapprox(deriv_angular_momentum, zeros(3), atol = 4.0e-15)

                    # Total energy conservation
                    function drho(
                            ::ContinuityDensity, ::WeaklyCompressibleSPHSystem,
                            particle
                        )
                        return dv[end, particle]
                    end
                    function drho(
                            ::ContinuityDensity, ::EntropicallyDampedSPHSystem,
                            particle
                        )
                    ...
                    function drho(::SummationDensity, system, particle)
                        return sum(
                            neighbor -> drho_particle(particle, neighbor),
                            1:n_particles
                        )
                    end
