@testset verbose = true "State Equations" begin
    ...
    @testset verbose = false "inverse_state_equation" begin
        ...
        state_equations = [
            StateEquationCole(
                sound_speed = 1484.0, exponent = 7.15, reference_density = 998.34,
                background_pressure = 101_325.0
            ),
            StateEquationCole(
                sound_speed = 10.0, exponent = 7, reference_density = 1000.0,
            ...
            StateEquationCole(
                sound_speed = 1484.0, exponent = 1, reference_density = 998.34,
                background_pressure = 101_325.0
            ),
            StateEquationCole(
                sound_speed = 10.0, exponent = 1, reference_density = 1000.0,
                background_pressure = 100_000.0
            ),
            StateEquationCole(
                sound_speed = 10.0, exponent = 1, reference_density = 1000.0,
        ...
        @testset "$state_equation" for state_equation in state_equations
            for density in densities
                ...
                @test TrixiParticles.inverse_state_equation(
                    state_equation,
                    pressure
                ) ≈ density
            end
            ...
            for pressure in pressures
                ...
                @test isapprox(state_equation(density), pressure, atol = 2.0e-7, rtol = 1.0e-10)
            end
        end
    end
    @testset verbose = true "StateEquationIdealGas" begin
        ...
        @testset "Physical Properties of Water" begin
            ...
            state_equation = StateEquationIdealGas(;
                sound_speed, gamma = gamma,
                reference_density = rest_density,
                background_pressure = 1ATM
            )
