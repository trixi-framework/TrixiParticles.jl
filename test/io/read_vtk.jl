@testset verbose = true "`vtk2trixi`" begin
    mktempdir() do tmp_dir
        ...
        expected_ic = InitialCondition(;
            coordinates = coordinates, velocity = velocity,
        ...
        @testset verbose = true "`InitialCondition`" begin
            ...
            @test isapprox(expected_ic.coordinates, test_ic.coordinates, rtol = 1.0e-5)
            @test isapprox(expected_ic.velocity, test_ic.velocity, rtol = 1.0e-5)
            @test isapprox(expected_ic.density, test_ic.density, rtol = 1.0e-5)
            @test isapprox(expected_ic.pressure, test_ic.pressure, rtol = 1.0e-5)
        end
        ...
        @testset verbose = true "`FluidSystem`" begin
            ...
            trixi2vtk(
                fluid_system, v_ode, u_ode, semi, 0.0,
            ...
            @test isapprox(u, test.coordinates, rtol = 1.0e-5)
            @test isapprox(v, test.velocity, rtol = 1.0e-5)
            @test isapprox(pressure, test.pressure, rtol = 1.0e-5)
            @test isapprox(fluid_system.cache.density, test.density, rtol = 1.0e-5)
        end
        ...
        @testset verbose = true "`BoundarySystem`" begin
            ...
            trixi2vtk(
                boundary_system, v_ode, u_ode, semi, 0.0,
            ...
            @test isapprox(boundary_system.coordinates, test.coordinates, rtol = 1.0e-5)
            # The velocity is always zero for `BoundarySystem`
            @test isapprox(zeros(size(test.velocity)), test.velocity, rtol = 1.0e-5)
            @test isapprox(boundary_model.pressure, test.pressure, rtol = 1.0e-5)
            @test isapprox(boundary_model.cache.density, test.density, rtol = 1.0e-5)
        end
    end
end
