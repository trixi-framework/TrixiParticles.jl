...
@testset verbose = true "Examples $TRIXIPARTICLES_TEST_" begin
    @testset verbose = true "Fluid" begin
        @trixi_testset "fluid/dam_break_2d_gpu.jl Float64" begin
            if Main.supports_double_precision
                @trixi_test_nowarn trixi_include(
                    @__MODULE__,
                    joinpath(
                        examples_dir(), "fluid",
                        "dam_break_2d_gpu.jl"
                ...
                @test_throws error trixi_include(
                    @__MODULE__,
                    joinpath(
                        examples_dir(), "fluid",
                        "dam_break_2d_gpu.jl"
        ...
        @trixi_testset "fluid/dam_break_2d_gpu.jl Float32" begin
            ...
            for (test_description, kwargs) in dam_break_tests
                @testset "$test_description" begin
                    ...
                    @trixi_test_nowarn trixi_include_changeprecision(
                        Float32, @__MODULE__,
                        joinpath(
                            examples_dir(),
                            "fluid",
        ...
        @trixi_testset "fluid/dam_break_3d.jl" begin
            ...
            semi_fullgrid = Semidiscretization(
        ...
        @trixi_testset "fluid/hydrostatic_water_column_2d.jl" begin
            ...
            hydrostatic_water_column_tests = Dict(
                ...
                "WCSPH with SchoenbergQuinticSplineKernel" => (
                    smoothing_length = 1.1,
                    smoothing_kernel = SchoenbergQuinticSplineKernel{2}(),
                ),
                "WCSPH with WendlandC2Kernel" => (
                    smoothing_length = 1.5,
                    smoothing_kernel = WendlandC2Kernel{2}(),
                ),
                "WCSPH with WendlandC4Kernel" => (
                    smoothing_length = 1.75,
                    smoothing_kernel = WendlandC4Kernel{2}(),
                ),
                "WCSPH with WendlandC6Kernel" => (
        ...
        @trixi_testset "fluid/pipe_flow_2d.jl - steady state reached (`dt`)" begin
            steady_state_reached = SteadyStateReachedCallback(;
                dt = 0.002f0,
                interval_size = 10,
                reltol = 1.0f-3
            )
    ...
    @testset verbose = true "FSI" begin
        @trixi_testset "fsi/dam_break_gate_2d.jl" begin
            ...
            cell_list = FullGridCellList(; min_corner, max_corner)
            semi_fullgrid = Semidiscretization(
                fluid_system, boundary_system_tank,
        ...
        @testset verbose = true "Interpolation" begin
            ...
            @testset verbose = true "Line" begin
                ...
                result = interpolate_line(
                    start_point, end_point, n_interpolation_points,
                    semi_new, semi_new.systems[1], sol;
                    cut_off_bnd = false
                )

                @test isapprox(
                    Array(result.computed_density),
                    Float32[
                        500.33255, 893.09766, 997.7032, 1001.14355, 1001.234,
                        1001.0098, 1000.4352, 999.7572, 999.1139, 989.6319,
                    ]
                )

                @test isapprox(
                    Array(result.density),
                    Float32[
                        1002.3152, 1002.19653, 1001.99915, 1001.7685,
                        1001.5382,
                ...
                @test isapprox(
                    Array(result.pressure),
                    Float32[
                        5450.902, 5171.2856, 4706.551, 4163.9185, 3621.5042,
            ...
            @testset verbose = true "Plane" begin
                ...
                @test isapprox(
                    Array(result.computed_density),
                    Float32[
                        250.18625, 500.34482, 499.77225, 254.3632, 499.58026,
                ...
                @test isapprox(
                    Array(result.density),
                    Float32[
                        1002.34467, 1002.3365, 1001.5021, 999.7109,
                        1000.84863,
                ...
                @test isapprox(
                    Array(result.pressure),
                    Float32[
                        5520.0513, 5501.1846, 3536.2256, -680.5194,
                        1997.7814,
