@testset verbose=true "Validation" begin
    ...
    @trixi_testset "dam_break_2d" begin
        ...
        if VERSION >= v"1.10"
            ...
            @test isapprox(error_edac_P1, 0, atol=1e-16)
            @test isapprox(error_edac_P2, 0, atol=1e-16)
            @test isapprox(error_wcsph_P1, 0, atol=1e-16)
            @test isapprox(error_wcsph_P2, 0, atol=1e-16)
        end
