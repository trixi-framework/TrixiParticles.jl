    ...
    @trixi_testset "Akinci cube-to-sphere 3D" begin
        @trixi_test_nowarn trixi_include(@__MODULE__,
                                         joinpath(validation_dir(),
                                                  ...
                                                  "validation.jl");
                                         baseline_particles_per_dimension=3,
                                         tspan=(0.0, 2.5e-5),
        ...
        @test size(shootout_results, 1) == 21
        ...
        cohesion_results = shootout_results[shootout_results.model .== "CohesionForceAkinci",
                                            :]
        full_akinci_results = shootout_results[shootout_results.model .== "SurfaceTensionAkinci",
                                               :]
        @test any(!isapprox(only(cohesion_results[cohesion_results.case .== row.case,
                                                  :kinetic_energy]),
                            row.kinetic_energy; rtol=1.0e-6)
                  for row in eachrow(full_akinci_results))
        @test count_rhs_allocations(sol) == 0
    end
end
