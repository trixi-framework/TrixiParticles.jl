@testset verbose = true "InitialCondition" begin
    disjoint_shapes_dict = Dict(
        "Rectangular Shapes" => (
            ...
            RectangularShape(
                0.1, (4, 5), (0.0, 1.0), density = 1.0,
                velocity = (0.3, -0.5)
            ),
        ),
        "Touching Rectangular Shapes" => (
            RectangularShape(
                0.1, (3, 4), (-1.0, 1.0),
                density = 1.0
            ),
        ...
        "Sphere Shapes" => (
            SphereShape(0.15, 0.5, (-1.0, 1.0), 1.0),
            SphereShape(0.15, 0.2, (0.0, 1.0), 1.0),
            SphereShape(
                0.15, 1.0, (0.0, -0.2), 1.0,
                sphere_type = RoundSphere()
            ),
        ),
        "Touching Mixed Shapes" => (
            RectangularShape(
                0.1, (3, 10), (-1.0, 0.0),
                density = 1.0
            ),
    ...
    @testset verbose = true "Constructors" begin
        @testset "Illegal Inputs" begin
            ...
            @test_throws ArgumentError(error_str) InitialCondition(
        ...
        @testset "Constant Quantities" begin
            ic_actual1 = InitialCondition(
                coordinates = zeros(2, 5), velocity = (1.0, 2.0),
                mass = 3.0, density = 4.0, pressure = 5.0
            )
            ic_actual2 = InitialCondition(
                coordinates = zeros(2, 5), velocity = [1.0, 2.0],
                mass = 3.0, density = 4.0, pressure = 5.0
            )
            ic_expected = InitialCondition(
                coordinates = zeros(2, 5),
        ...
        @testset "Automatic Mass Calculation" begin
            ...
            coordinates = [
                88.3 10.4 5.2 48.3 58.9;
                23.6 92.5 92.1 96.7 84.8;
                77.5 44.1 18.2 30.5 44.0
            ]
            ic_actual = InitialCondition(;
                coordinates, velocity = x -> 2x,
                density = x -> 4x[2], pressure = x -> 5x[3],
                particle_spacing
            )
            ic_expected = InitialCondition(;
                coordinates, velocity = 2coordinates,
        ...
        @testset "Quantities as Functions" begin
            ...
            ic_actual = InitialCondition(;
                coordinates, velocity = x -> 2x, mass = x -> 3x[1],
                density = x -> 4x[2], pressure = x -> 5x[3]
            )
            ic_expected = InitialCondition(;
                coordinates, velocity = 2coordinates,
    ...
    @testset verbose = true "Union of Overlapping Shapes" begin
        @testset "Rectangular Shapes" begin
            shape1 = RectangularShape(
                0.13, (9, 10), (0.0, 0.0), density = 1.0,
                loop_order = :x_first
            )
            shape2 = RectangularShape(
                0.13, (4, 5), (1.0, 1.0), density = 1.0,
                loop_order = :x_first
            )
    ...
    @testset verbose = true "Intersect of Overlapping Shapes" begin
        ...
        @testset "Rectangle and RoundSphere" begin
            ...
            expected_coords = [
                -0.35 -0.35 -0.25 -0.25 -0.25 -0.25 -0.25 -0.25 -0.15 -0.15 -0.15 -0.15 -0.15 -0.15 -0.05 -0.05 -0.05 -0.05 -0.05 -0.05 -0.05 -0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.15 0.15 0.15 0.15 0.15 0.15 0.25 0.25 0.25 0.25 0.25 0.25 0.35 0.35;
        ...
        @testset "RoundSphere and Rectangle" begin
            ...
            expected_coords = [
                0.8 0.75 0.65 0.6 0.65 0.75 0.81361295 0.72410734 0.62907902 0.55029785 0.50581164 0.50581164 0.55029785 0.62907902 0.72410734 0.81361295 0.77364565 0.6752262 0.57949137 0.49681553 0.43615787 0.40409161 0.40409161 0.43615787 0.49681553 0.57949137 0.6752262 0.77364565;
        ...
        @testset "Rectangle and Low-Res Sphere" begin
            ...
            expected_coords = [
                -0.25 -0.25 -0.15 -0.15 -0.05 -0.05 -0.05 -0.05 -0.05 -0.05 0.05 0.05 0.05 0.05 0.05 0.05 0.15 0.15 0.25 0.25;
