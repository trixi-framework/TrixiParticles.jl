@testset verbose = true "Smoothing Kernels" begin
    ...
    @testset verbose = false "Integral" begin
        ...
        function integrate_kernel_2d(smk)
            integral_2d_radial,
                _ = quadgk(
                r -> r * TrixiParticles.kernel(smk, r, 1.0), 0,
        ...
        function integrate_kernel_3d(smk)
            integral_3d_radial,
                _ = quadgk(
                r -> r^2 * TrixiParticles.kernel(smk, r, 1.0), 0,
                TrixiParticles.compact_support(smk, 1.0),
                rtol = 1.0e-15
            )
        ...
        @testset "GaussianKernel" begin
            ...
            @test 1.0e-4 < error_2d < 1.0e-3
            @test 1.0e-4 < error_3d < 1.0e-3
        end
        ...
        @testset "$kernel" for kernel in kernels
            ...
            @test error_2d <= 1.0e-15
            @test error_3d <= 1.0e-15
        end
    end
            ...
            for ndims in 2:3
                ...
                for h in smoothing_lengths
                    ...
                    for r in radii
                        ...
                        @test isapprox(
                            analytic_deriv, automatic_deriv,
                            rtol = 5.0e-15, atol = 4 * eps()
                        )
                    end
                end
            end
