...
@trixi_testset "Semidiscretization" begin
    ...
    @testset verbose = true "Constructor" begin
        ...
        nhs = (
            (
                TrixiParticles.TrivialNeighborhoodSearch{3}(
                    search_radius = 0.2,
                    eachpoint = 1:2
                ),
                TrixiParticles.TrivialNeighborhoodSearch{3}(
    ...
    @testset verbose = true "Check Configuration" begin
        @testset verbose = true "Solid-Fluid Interaction" begin
            ...
            ic = InitialCondition(;
                particle_spacing = 1.0, coordinates = ones(2, 2),
                density = [1.0, 1.0]
            )

            fluid_system = FluidSystemMock()
            model_a = BoundaryModelMock()
            model_b = BoundaryModelDummyParticles(
                [1.0], [1.0], ContinuityDensity(), kernel,
                1.0
            )
            ...
            @test_throws ArgumentError(error_str) Semidiscretization(
                fluid_system,
                solid_system1,
            ...
            solid_system2 = TotalLagrangianSPHSystem(
                ic, kernel, 1.0, 1.0, 1.0,
                boundary_model = model_a
            )

            @test_nowarn TrixiParticles.check_configuration(
                (solid_system2, fluid_system),
                nothing
            )

            # FSI with wrong boundary model
            solid_system3 = TotalLagrangianSPHSystem(
                ic, kernel, 1.0, 1.0, 1.0,
