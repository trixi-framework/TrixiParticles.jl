@testset verbose = true "SPH Interpolation" begin
    function compare_interpolation_result(actual, expected; tolerance = 5.0e-4)
        ...
        for i in 1:length(expected.density)
            ...
            if !(all(isnan.(actual.velocity[:, i])) && all(isnan.(expected.velocity[:, i])))
                @test isapprox(
                    actual.velocity[:, i], expected.velocity[:, i],
                    atol = tolerance
                )
            end
        end
    ...
    function binary_search_outside(inside, outside, func = nothing; tolerance = 1.0e-5)
    ...
    @testset verbose = true "2D" begin
        ...
        state_equation = StateEquationCole(;
            sound_speed, reference_density = 1000.0,
            exponent = 7, clip_negative_pressure = false
        )

        # Create fluid starting at y = 0
        fluid = rectangular_patch(
            particle_spacing, (nx, ny), seed = 1,
        ...
        bnd = rectangular_patch(
            particle_spacing, (bnd_nx, bnd_ny), seed = 1,
            perturbation_factor = 0.0, perturbation_factor_position = 0.0,
        ...
        for cut_off_bnd in [true, false]
            @testset verbose = true "Interpolation Point no boundary - cut_off_bnd = $(cut_off_bnd)" begin
                interpolation_walldistance(y) = TrixiParticles.interpolate_points(
                    [0.0; y;;],
                    semi_no_boundary,
                    fluid_system,
                    v_no_bnd,
                ...
                # Top outside
                distance_top_outside = binary_search_outside(
                    ny * particle_spacing,
                    (ny + 2) * particle_spacing,
                    interpolation_walldistance
                )
                @test isapprox(distance_top_outside, 0.11817626953124982, atol = 1.0e-14)

                result_zero = interpolation_walldistance(
                    ny * particle_spacing +
                        distance_top_outside
                )
                compare_interpolation_result(
                    result_zero,
                    expected_zero(
                        ny * particle_spacing +
                            distance_top_outside
                ...
                result_top_outside = interpolation_walldistance(
                    ny * particle_spacing +
                ...
                distance_bottom_outside = binary_search_outside(
                    0.0, -2 * particle_spacing,
                    interpolation_walldistance
                )
                @test isapprox(distance_bottom_outside, 0.11817626953125, atol = 1.0e-14)
                ...
                result_bottom_outside = interpolation_walldistance(
                    -0.5 *
                ...
                result_multipoint = TrixiParticles.interpolate_points(
                    multi_point_coords,
                    semi_no_boundary,
                    fluid_system,
                    v_no_bnd, u_no_bnd,
                ...
                expected_multi = (
                    density = [666.0, 666.0000000000001, 666.0],
            ...
            @testset verbose = true "Interpolation Line no boundary - cut_off_bnd = $(cut_off_bnd)" begin
                result_endpoint = TrixiParticles.interpolate_line(
                    [1.0, -0.05], [1.0, 1.0],
                    5, semi_no_boundary,
                    fluid_system,
                    v_no_bnd, u_no_bnd,
                ...
                result = TrixiParticles.interpolate_line(
                    [1.0, -0.05], [1.0, 1.0],
                    5, semi_no_boundary,
                ...
                expected_res = (
                    density = [666.0, 666.0, 666.0], neighbor_count = [2, 2, 1],
                    point_coords = [
                        1.0 1.0 1.0;
                        0.2125 0.47500000000000003 0.7375
                    ],
                    velocity = [
                        7.699999999999999 7.7 7.7;
                ...
                expected_res_end = (
                    density = [666.0, 666.0, 666.0, 666.0, 666.0],
                    ...
                    velocity = [
                        7.7 7.699999999999999 7.699999999999999 7.7 7.7;
            ...
            @testset verbose = true "Interpolation Plane no boundary - cut_off_bnd = $(cut_off_bnd)" begin
                ...
                expected_res = (
                    ...
                    point_coords = [
                        0.25 0.5 0.75 0.0 0.25 0.5 0.75 1.0 0.0 0.25 0.5 0.75 1.0 0.0 0.25 0.5 0.75 1.0 0.0 0.25 0.5 0.75;
                        0.0 0.0 0.0 0.25 0.25 0.25 0.25 0.25 0.5 0.5 0.5 0.5 0.5 0.75 0.75 0.75 0.75 0.75 1.0 1.0 1.0 1.0
                    ],
                    velocity = [
                        5.9 6.499999999999999 7.1000000000000005 4.999999999999994 5.9 6.5 7.1000000000000005 7.7 4.999999999999998 5.9 6.5 7.1 7.7 4.999999999999995 5.900000000000001 6.5 7.1000000000000005 7.7 5.0 5.8999999999999995 6.5 7.100000000000001;
        ...
        for cut_off_bnd in [true, false]
            @testset verbose = true "Interpolation Point boundary - cut_off_bnd = $(cut_off_bnd)" begin
                ...
                if cut_off_bnd
                    @test isapprox(distance_bottom_outside, 0, atol = 1.0e-12)
                else
                    @test isapprox(distance_bottom_outside, 0.11817392366574496, atol = 1.0e-14)
                end
            ...
            @testset verbose = true "Interpolation Line boundary - cut_off_bnd = $(cut_off_bnd)" begin
                ...
                if cut_off_bnd
                    ...
                    expected_res_end = (
                        ...
                        velocity = [
                            NaN 7.699999999999999 7.699999999999999 7.7 7.7;
    ...
    @testset verbose = true "3D" begin
        ...
        fluid = rectangular_patch(
            particle_spacing, (nx, ny, nz), seed = 1,
            perturbation_factor = 0.0, perturbation_factor_position = 0.0,
            set_function = set_values,
            offset = [0.0, ny * 0.5 * particle_spacing, 0.0]
        )
        bnd = rectangular_patch(
            particle_spacing, (bnd_nx, bnd_ny, bnd_nz), seed = 1,
            perturbation_factor = 0.0, perturbation_factor_position = 0.0,
            set_function = set_values_bnd,
        ...
        # some simple results
        expected_zero(y) = (
            density = [NaN], neighbor_count = [0], point_coords = [0.0; y; 0.0;;],
        ...
        for cut_off_bnd in [true, false]
            @testset verbose = true "Interpolation Point no boundary - cut_off_bnd = $(cut_off_bnd)" begin
                ...
                @test isapprox(distance_top_outside, 0.09390704035758901, atol = 1.0e-14)
                ...
                @test isapprox(distance_bottom_outside, 0.0939070362292114, atol = 1.0e-14)
                ...
                expected_multi = (
                    density = [666.0, 666.0, 666.0], neighbor_count = [4, 4, 9],
                    point_coords = multi_point_coords,
                    velocity = [
                        5.0 5.0 5.0;
        ...
        for cut_off_bnd in [true, false]
            @testset verbose = true "Interpolation Point boundary - cut_off_bnd = $(cut_off_bnd)" begin
                ...
                @test isapprox(distance_top_outside, 0.09390869140624947, atol = 1.0e-14)
            ...
            @testset verbose = true "Interpolation Plane no boundary - cut_off_bnd = $(cut_off_bnd)" begin
                ...
                expected_res = (
                    ...
                    velocity = [
                        5.0 5.0 5.0 5.920251592989799 5.912901655029027 5.920251592989798 7.079748407010201 7.087098344970975 7.079748407010202 7.699999999999998 7.700000000000001 7.699999999999999;
