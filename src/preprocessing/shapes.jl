...
function ComplexShape(; filename, particle_spacing, density, velocity=nothing,
                      ...
                      point_in_poly_algorithm=WindingNumberJacobson(), seed=nothing,
    ...
    elseif file_extension == ".stl"
        if point_in_poly_algorithm isa WindingNumberHorman
    ...
    return sample(; shape, particle_spacing, density, velocity, point_in_poly_algorithm,
...
function sample(; shape, particle_spacing, density, velocity=zeros(ndims(shape)),
                pressure=0.0, point_in_poly_algorithm=WindingNumberJacobson(),
    ...
    inpoly = point_in_poly_algorithm(shape, grid)
