...
function init_nhs!(semi, u_ode)
    ...
    foreach_system(semi) do system
        ...
        foreach_system(semi) do neighbor
            ...
            init_nhs!(neighborhood_search, system, neighbor, u_system, u_neighbor, semi)
        end
    end
end
...
# To prevent hard-to-find bugs, there is no default version
# -- Fluid / structure interactions
function init_nhs!(neighborhood_search,
                     system::AbstractFluidSystem,
                     neighbor::Union{AbstractFluidSystem, TotalLagrangianSPHSystem,
                                     RigidBodySystem},
                     u_system, u_neighbor, semi)
...
# === PointNeighbors forwarding ===
function initialize!(neighborhood_search, x, y, semi; points_moving=(true, true),
                 eachindex_y=axes(y, 2))
    PointNeighbors.initialize!(neighborhood_search, x, y; eachindex_y,
                           parallelization_backend=semi.parallelization_backend)
end
