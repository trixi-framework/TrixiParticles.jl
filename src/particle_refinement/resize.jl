...
@inline function resize_system!(system::FluidSystem,
    ...
    (; system_child) = particle_refinement

    candidates_refine = length(particle_refinement.candidates)
    candidates_coarsen = length(system_child.particle_coarsening.ranges)
    waste_children = length(system_child.particle_coarsening.candidates)
...
@inline get_iterator(system::FluidSystem) = get_iterator(system,
                                                         system.particle_refinement,
...
@inline function get_iterator(system::FluidSystem,
    ...
    # Uncomment for benchmark
    # return Iterators.filter(i -> !(i in candidates), eachparticle(system))
    return setdiff(eachparticle(system), candidates_coarsening)
end
