...
function write2vtk!(vtk, v, u, t, model::BoundaryModelDummyParticles, system;
    ...
    if write_meta_data
        ...
        vtk["viscosity_model"] = type2string(system.viscosity)
    end

    vtk["hydrodynamic_density"] = [particle_density(v, system, particle)
                                   for particle in eachparticle(system)]
    vtk["pressure"] = model.pressure
    if system.viscosity isa ViscosityAdami
        vtk["wall_velocity"] = view(system.cache.wall_velocity, 1:ndims(system), :)
    end
end
