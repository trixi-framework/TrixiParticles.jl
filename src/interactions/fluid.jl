    ...
    @threaded for particle in each_moving_particle(particle_container)
        ...
        for neighbor in eachneighbor(particle_coords, neighborhood_search)
            ...
            if eps() < distance2 <= compact_support(particle_container)^2
                ...
                dv_pressure = calc_momentum_eq(particle, particle_container, neighbor,
                ...
                for i in 1:ndims(particle_container)
                    dv[i, particle] += dv_boundary[i] + dv_pressure[i] + dv_viscosity[i]
                end
...
@inline function calc_momentum_eq(particle, particle_container::FluidParticleContainer,
                                  neighbor,
                                  neighbor_container::FluidParticleContainer, m_b, rho_a,
                                  rho_b, grad_kernel)
