...
@inline function update_transport_velocity!(system::FluidSystem, v_ode, semi)
    return update_transport_velocity!(system, v_ode, semi, system.transport_velocity)
end
...
@inline function momentum_convection(
        system, neighbor_system, ::Nothing,
...
@inline function momentum_convection(
        system, neighbor_system, ::TransportVelocityAdami,
        v_particle_system, v_neighbor_system, rho_a, rho_b,
        m_a, m_b, particle, neighbor, grad_kernel
    )
    volume_a = m_a / rho_a
...
@inline transport_velocity!(
    dv, system, neighbor_system, particle, neighbor, m_a, m_b,
    grad_kernel
) = dv

@inline function transport_velocity!(
        dv, system::FluidSystem, neighbor_system,
        particle, neighbor, m_a, m_b, grad_kernel
    )
    return transport_velocity!(
        dv, system, neighbor_system, system.transport_velocity,
        particle, neighbor, m_a, m_b, grad_kernel
    )
end

@inline function transport_velocity!(
        dv, system, neighbor_system, ::Nothing,
        particle, neighbor, m_a, m_b, grad_kernel
    )
    return dv
end
...
@inline function transport_velocity!(
        dv, system, neighbor_system, ::TransportVelocityAdami,
        particle, neighbor, m_a, m_b, grad_kernel
    )
...
function reset_callback_flag!(system::FluidSystem)
    return reset_callback_flag!(system, system.transport_velocity)
end
...
function update_callback_used!(system::FluidSystem)
    return update_callback_used!(system, system.transport_velocity)
end

update_callback_used!(system, ::Nothing) = system

function update_callback_used!(system, transport_velocity)
    return system.cache.update_callback_used[] = true
end
...
function check_tvf_configuration(
        system::FluidSystem, ::TransportVelocityAdami,
        v, u, v_ode, u_ode, semi, t; update_from_callback = false
    )
