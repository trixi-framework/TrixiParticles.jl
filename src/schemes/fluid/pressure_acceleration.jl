...
# asymmetric version.
@inline function pressure_acceleration_summation_density(
        m_a, m_b, rho_a, rho_b, p_a, p_b,
        W_a
    )
...
# We assume symmetry of the kernel gradient in this formulation. See below for the
# asymmetric version.
@inline function pressure_acceleration_continuity_density(
        m_a, m_b, rho_a, rho_b, p_a, p_b,
        W_a
    )
...
function choose_pressure_acceleration_formulation(
    ...
    if correction isa KernelCorrection ||
        ...
        if isempty(
            ...
            throw(
                ArgumentError(
                    "when a correction with an asymmetric kernel gradient is " *
        ...
        if isempty(
            ...
            throw(
                ArgumentError(
                    "when not using a correction with an asymmetric kernel " *
