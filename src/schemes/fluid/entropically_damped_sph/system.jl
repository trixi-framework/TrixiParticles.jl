...
@inline function initial_velocity(system, particle, initial_velocity_function)
    position = initial_coords(system, particle)
    return SVector(ntuple(i -> initial_velocity_function[i](position), Val(ndims(system))))
end
