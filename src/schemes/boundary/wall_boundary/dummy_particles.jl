...
function create_cache_model(initial_density,
    ...
    return (; reference_density, density, a_ii, predicted_density, sum_d_ij_pj, sum_term, omega, time_step, density_error)
end
...
function compute_pressure!(boundary_model, ::Union{PressureMirroring, PressureZeroing, PressureBoundaries},
                           system, v, u, v_ode, u_ode, semi)
...
function calculate_diagonal_elements(system::AbstractBoundarySystem, v, u,
    ...
    (; boundary_model) = system
    (; density_calculator) = boundary_model
    return calculate_diagonal_elements(system, boundary_model, density_calculator, v, u, v_ode, u_ode, semi, t)
end

function calculate_diagonal_elements(system, boundary_model, density_calculator, v, u,
    v_ode, u_ode, semi, t)
    return system
end

function calculate_diagonal_elements(system, boundary_model, density_calculator::PressureBoundaries, v, u,
    v_ode, u_ode, semi, t)
    (; a_ii, time_step) = boundary_model.cache
...
function calculate_diagonal_elements!(a_ii, system, boundary_model, density_calculator, neighbor_system::AbstractFluidSystem, v, u, v_ode, u_ode, semi, time_step)
...
function calculate_predicted_density(system::AbstractBoundarySystem, v, u,
    ...
    (; boundary_model) = system
    (; density_calculator) = boundary_model
    return calculate_predicted_density(system, boundary_model, density_calculator, v, u, v_ode, u_ode, semi, t)
end

# Calculate the predicted density (with the continuity equation and predicted velocities)
function calculate_predicted_density(system, boundary_model, density_calculator, v, u,
    v_ode, u_ode, semi, t)
    return system
end

function calculate_predicted_density(system, boundary_model, ::PressureBoundaries, v, u, v_ode, u_ode, semi, t)
...
function calculate_sum_term_values(system::AbstractBoundarySystem, u, u_ode, semi)
    (; boundary_model) = system
    (; density_calculator) = boundary_model
    return calculate_sum_term_values(system, boundary_model, density_calculator, u, u_ode, semi)
end

function calculate_sum_term_values(system, boundary_model, density_calculator, u, u_ode, semi)
    return system
end

function calculate_sum_term_values(system, boundary_model, ::PressureBoundaries, u, u_ode, semi)
...
#TODO: Same function exists in system.jl from IISPH, one can be deleted
function calculate_sum_term!(sum_term, system::AbstractBoundarySystem, neighbor_system, u, u_ode, semi, time_step)
    # Get neighbor system u and v values
    u_neighbor_system = wrap_u(u_ode, neighbor_system, semi)
    # Get coordinates
    system_coords = current_coordinates(u, system)
...
function pressure_update(system, boundary_model, ::PressureBoundaries, u, u_ode, semi)
    ...
    total_density_error = zero(eltype(system))
    @threaded semi for particle in eachparticle(system)
    ...
    total_density_error = sum(density_error) / reference_density

    return total_density_error
end
...
function calculate_d_ij(system::AbstractBoundarySystem, neighbor_system, particle_j, grad_kernel, time_step)
    (; boundary_model) = system
    (; density_calculator) = boundary_model

    return calculate_d_ij(system, density_calculator, neighbor_system, particle_j, grad_kernel, time_step)
end

# Calculates the d_ij value for a particle i and his neighbor j from the equation 9 in 'IHMSEN et al'
function calculate_d_ij(system, density_calculator, neighbor_system::AbstractFluidSystem, particle_j, grad_kernel, time_step)
    return system
end

#TODO Check if d_ij is even needed, when particle i is a  boundary particle
# Calculates the d_ij value for a particle i and his neighbor j from the equation 9 in 'IHMSEN et al'
function calculate_d_ij(system, ::PressureBoundaries, neighbor_system::AbstractFluidSystem, particle_j, grad_kernel, time_step)
    # (delta t)^2 * m_i / rho_i ^2 * gradW_ij
    println("This function gets called when using PressureBoundaries")
    return -time_step^2 * hydrodynamic_mass(neighbor_system, particle_j) /
    neighbor_system.density[particle_j]^2 * grad_kernel
end

# Calculates the d_ij value for a particle i and his neighbor j from the equation 9 in 'IHMSEN et al'
function calculate_d_ij(system, ::PressureBoundaries, neighbor_system::AbstractBoundarySystem,
    particle_j, grad_kernel,
    time_step)
    return zero(SVector{ndims(system), eltype(system)})
end

function calculate_d_ji(system::AbstractBoundarySystem, neighbor_system, particle_i, grad_kernel, time_step)
    (; boundary_model) = system
...
# Calculate the large sum in eq. 13 of Ihmsen et al. (2013) for each particle (as `sum_term`)
function calculate_sum_term(system, boundary_model, ::PressureBoundaries, neighbor_system::AbstractFluidSystem,
        particle, neighbor, grad_kernel, time_step)
...
function calculate_sum_term(system, boundary_model, ::PressureBoundaries, neighbor_system::AbstractBoundarySystem,
    particle, neighbor, grad_kernel, time_step)
    return 0
end
