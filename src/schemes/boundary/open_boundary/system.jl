...
struct OpenBoundarySPHSystem{
        BM, ELTYPE, NDIMS, IC, FS, FSI, ARRAY1D, BC, FC, BZ, RV,
        RP, RD, B, UCU, C,
    } <: System{NDIMS}
    boundary_model::BM
...
function OpenBoundarySPHSystem(
        boundary_model, initial_condition, fluid_system,
    ...
    return OpenBoundarySPHSystem{
        typeof(boundary_model), eltype(mass), ndims(initial_condition),
...
function OpenBoundarySPHSystem(
    ...
    if !(
            reference_velocity isa Function || isnothing(reference_velocity) ||
                (reference_velocity isa Vector && length(reference_velocity) == NDIMS)
        )
        throw(
            ArgumentError(
                "`reference_velocity` must be either a function mapping " *
    ...
    if !(
            reference_pressure isa Function || reference_pressure isa Real ||
                isnothing(reference_pressure)
        )
        throw(
            ArgumentError(
                "`reference_pressure` must be either a function mapping " *
    ...
    if !(
            reference_density isa Function || reference_density isa Real ||
                isnothing(reference_density)
        )
        throw(
            ArgumentError(
                "`reference_density` must be either a function mapping " *
    ...
    cache = create_cache_open_boundary(
        boundary_model, initial_condition,
...
function create_cache_open_boundary(
        boundary_model, initial_condition,
        reference_density, reference_velocity,
        reference_pressure
    )
...
function update_final!(
        system::OpenBoundarySPHSystem, v, u, v_ode, u_ode, semi, t;
    ...
    return update_final!(system, system.boundary_model, v, u, v_ode, u_ode, semi, t)
end

# This function is called by the `UpdateCallback`, as the integrator array might be modified
function update_open_boundary_eachstep!(
        system::OpenBoundarySPHSystem, v_ode, u_ode,
        semi, t
    )
    ...
    @trixi_timeit timer() "update boundary quantities" update_boundary_quantities!(
        system,
...
function check_domain!(system, v, u, v_ode, u_ode, semi)
    ...
    @threaded semi for i in eachindex(crossed_boundary_particles)
        ...
        convert_particle!(
            system, fluid_system, boundary_zone, particle, particle_new,
            v, u, v_fluid, u_fluid
        )
    end
    ...
    @threaded semi for i in eachindex(crossed_fluid_particles)
        ...
        convert_particle!(
            fluid_system, system, boundary_zone, particle, particle_new,
            v, u, v_fluid, u_fluid
        )
    end
...
@inline function convert_particle!(
        system::OpenBoundarySPHSystem, fluid_system,
...
@inline function convert_particle!(
        fluid_system::FluidSystem, system,
...
@inline function transfer_particle!(
        system_new, system_old, particle_old, particle_new,
...
@inline function viscosity_model(
