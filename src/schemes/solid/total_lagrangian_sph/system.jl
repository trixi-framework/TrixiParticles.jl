...
function von_mises_stress(system)
    ...
    @threaded default_backend(von_mises_stress_vector) for particle in each_moving_particle(system)
...
function cauchy_stress(system::TotalLagrangianSPHSystem)
    ...
    @threaded default_backend(cauchy_stress_tensors) for particle in each_moving_particle(system)
        F = deformation_gradient(system, particle)
        J = det(F)
        P = pk1_corrected(system, particle)
        sigma = (1.0 / J) * P * F'
