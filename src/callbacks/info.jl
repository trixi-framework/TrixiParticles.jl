...
function Base.show(io::IO, cb::DiscreteCallback{<:Any, <:InfoCallback})
    ...
    callback = cb.affect!
    return print(io, "InfoCallback(interval=", callback.interval, ")")
end
...
function InfoCallback(; interval = 0, reset_threads = true)
    ...
    function initialize(cb, u, t, integrator)
        return initialize_info_callback(
            cb, u, t, integrator;
            reset_threads
        )
    end
...
function (info_callback::InfoCallback)(integrator)
    if isfinished(integrator)
        ...
        println(
            rpad(
                @sprintf(
                    "#timesteps: %6d │ Δt: %.4e │ sim. time: %.4e (%5.3f%%)",
...
function initialize_info_callback(
        discrete_callback, u, t, integrator;
        reset_threads = true
    )
...
function format_key_value_line(
        key::AbstractString, value::AbstractString, key_width,
        total_width;
...
function summary_header(io, heading; total_width = 100, indentation_level = 0)
    ...
    return println(io, "│ " * "═"^length(heading) * " "^(total_width - length(heading) - 4) * " │")
end
...
function summary_line(io, key, value; key_width = 30, total_width = 100, indentation_level = 0)
    ...
    s = format_key_value_line(
        key, value, key_width, total_width,
...
function print_summary(integrator)
    println("─"^100)
    println(
        "Trixi simulation finished.  Final time: ", integrator.t,
