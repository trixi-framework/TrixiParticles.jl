...
The capillary restriction ``\Delta t_\sigma`` is applied when using
[`SurfaceTensionAkinciCohesionPhysical`](@ref), [`SurfaceTensionMorris`](@ref), or
[`SurfaceTensionMomentumMorris`](@ref). For an interaction between two fluid systems, both
systems must use the same model. Then ``\bar{\rho} = (\rho_a + \rho_b) / 2``, ``h`` is the
smaller smoothing length, and ``\sigma`` is the larger surface tension coefficient. The
physical Akinci model uses its configured reference density; the Morris models use the current
density.
