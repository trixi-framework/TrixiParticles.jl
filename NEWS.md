...
- Added 2D/3D free-surface reconstruction from fluid particles with `SurfaceReconstruction`,
  `reconstruct_surface`, `reconstruct_surface!`, and `SurfaceReconstructionCallback`,
  producing closed area-corrected contours or volume-corrected triangle meshes. The
  callback writes VTK PolyData time series (PLY optional); meshes can be reconstructed
  offline from particle data, systems, solutions, and initial conditions.
...
- Added configurable system-pair interactions through the `interaction_matrix` keyword
  of `Semidiscretization`, allowing interactions to be disabled or replaced with custom
  interaction functions (#1057).
...
- Migrated to the new SciML stack with SciMLBase 3, dropping support for older versions
  of the SciML ecosystem (#1171).
- `DensityReinitializationCallback` now requires the `Semidiscretization` as second
  positional argument, i.e. `DensityReinitializationCallback(system, semi; ...)`.
  The previous one-argument syntax was removed because it did not behave as expected (#1181).
...
- Added vortex street example (#1221).

### Important Bugfixes

- Fixed density reinitialization for systems with inactive buffer particles (#1181).
- Fixed `SteadyStateReachedCallback` interval handling (#1180).
