...
- `DensityReinitializationCallback` now requires the `Semidiscretization` as second
  ...
  The previous one-argument syntax was removed because it reinitialized all eligible
  systems instead of only the passed system.
...
- Fixed density reinitialization for systems with inactive buffer particles.
