...
for density_calculator in density_calculators, perturbation in perturb_coordinates,
    ...
    pp_callback = PostprocessCallback(; dt=0.02, L1v=compute_l1v_error,
                                      L1p=compute_l1p_error, output_directory,
                                      filename="errors", write_csv=true,
                                      write_file_interval=1)

    # Import variables into scope
    trixi_include(@__MODULE__,
                  joinpath(examples_dir(), "fluid", "taylor_green_vortex_2d.jl");
                  density_calculator, perturb_coordinates=perturbation, wcsph,
                  particle_spacing, reynolds_number, tspan, saving_callback, pp_callback)
end
