...
function interpolated_velocity(system::TrixiParticles.FluidSystem, v, u, semi, t)
    ...
    values_y = interpolate_line(
        [0.5, 0.0], [0.5, 1.0], n_particles_xy, semi, system,
        v, u; endpoint = true, cut_off_bnd = true
    )

    values_x = interpolate_line(
        [0.0, 0.5], [1.0, 0.5], n_particles_xy, semi, system,
        v, u; endpoint = true, cut_off_bnd = true
    )
    ...
    if CAPTURE_STARTED[]
        ...
        df = TrixiParticles.DataFrame(
            pos = data.pos, counter = n_evaluations,
...
for reynolds_number in reynolds_numbers,
    ...
    global output_directory = joinpath(
        "out_ldc", wcsph ? "wcsph" : "edac",
