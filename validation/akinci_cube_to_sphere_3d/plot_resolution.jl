...
Label(figure[3, 1:2],
      ...
      "normal-force reference h: " *
      @sprintf("%.3e", only(unique(results.reference_smoothing_length)));
      fontsize=16, color=:gray35, padding=(0, 0, 0, 8))
Label(figure[4, 1:2],
      "Empirical sensitivity only: the normal term uses symmetric volume normalization and a " *
      "fixed reference h; no physical convergence is claimed.";
