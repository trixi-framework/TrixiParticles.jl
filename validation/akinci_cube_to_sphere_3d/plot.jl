...
expected_rows = length(case_order) * length(model_order)
...
function result_matrix(f)
    return [f(only(eachrow(results[(results.case .== case) .& (results.model .== model), :])))
...
is_best_configuration = result_matrix() do row
...
energy_range = extrema(log_kinetic_energy)
