...
edac_reference_files = glob(
    "validation_reference_edac*.json",
    case_dir
)
edac_sim_files = glob(
    "validation_result_dam_break_edac*.json",
    "out/"
)
...
wcsph_reference_files = glob(
    "validation_reference_wcsph*.json",
    case_dir
)
wcsph_sim_files = glob(
    "validation_result_dam_break_wcsph*.json",
    "out/"
)
...
function plot_experiment(
        ax, time, data, label, color = :black, marker = :utriangle,
        markersize = 6
    )
    return scatter!(ax, time, data; color, marker, markersize, label)
end

function plot_simulation(ax, time, data, label, color = :red, linestyle = :dash, linewidth = 3)
    return lines!(ax, time, data; color, linestyle, linewidth, label)
end
...
for ax_max in [ax_max_x_edac, ax_max_x_wcsph]
    ...
    ylims!(ax_max, 1, 3.0)
    plot_experiment(
        ax_max, surge_front.time, surge_front.surge_front,
...
Legend(
    fig[6, 1], ax_max_x_edac, tellwidth = false, orientation = :horizontal, valign = :top,
    nbanks = 2
)
Legend(
    fig[6, 2], ax_max_x_wcsph, tellwidth = false, orientation = :horizontal, valign = :top,
    nbanks = 2
)
