...
ref = CSV.read(
    joinpath(validation_dir(), "oscillating_beam_2d/reference_turek.csv"),
    DataFrame
)

# Get the list of JSON files
reference_files = glob(
    "validation_reference_*.json",
...
for file_name in input_files
    ...
    matching_keys_x = sort(
        collect(
            filter(
                key -> occursin(key_pattern_x, key),
                keys(json_data)
            )
        )
    )
    matching_keys_y = sort(
        collect(
            filter(
                key -> occursin(key_pattern_y, key),
                keys(json_data)
            )
        )
    )
...
# Plot reference data
lines!(
    ax1, ref.time, ref.Ux, color = :black, linestyle = :dash,
    label = "Turek and Hron 2006"
)
lines!(
    ax2, ref.time, ref.Uy, color = :black, linestyle = :dash,
    label = "Turek and Hron 2006"
)
