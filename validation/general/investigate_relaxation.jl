...
pp_cb = PostprocessCallback(;
    ekin = kinetic_energy, max_pressure, avg_density, dt = 0.025,
    filename = "relaxation", write_csv = false, write_file_interval = 0
)

pp_damped_cb = PostprocessCallback(;
    ekin = kinetic_energy, max_pressure, avg_density,
    dt = 0.025,
...
if file_path != ""
    ...
    tl_avg_rho_damped,
        grad_avg_rho_damped = calculate_regression(
        avg_rho_damped,
        time_damped
    )

    plot1 = Plots.plot(
        time, [e_kin, tl_ekin], label = ["undamped" "trend"],
        color = [:blue :red], linewidth = [2 2]
    )

    Plots.plot!(
        time_damped, [e_kin_damped, tl_ekin_damped],
    ...
    plot2 = Plots.plot(
        time, [p_max, tl_p_max], label = ["sim" "trend"], color = [:blue :red],
        linewidth = [2 2]
    )

    Plots.plot!(
        time_damped, [p_max_damped, tl_p_max_damped],
    ...
    plot3 = Plots.plot(
        time, [avg_rho, tl_avg_rho], label = ["sim" "trend"],
        color = [:blue :red], linewidth = [2 2]
    )
